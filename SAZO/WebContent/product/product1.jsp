...
<script type="text/javascript" src="<%=cp %>/js/sazo.js"></script>
...
function init () {
	sell_price = document.basket.sell_price.value;
	amount = document.form.amount.value;
	document.basket.sum.value = sell_price;
	change();
}

function add () {
	hm = document.basket.amount;
	sum = document.basket.sum;
	hm.value ++ ;

	sum.value = parseInt(hm.value) * ${dto.price};
}
...
<body onload="change();">
	...
	<div id="cart">
		<div class="th">
			<img src="${imagePath }${dto.itemNum }/${dto.thumFile}" alt="썸네일 이미지" class="thum" />
		</div>
		<form name="basket" method="post">
			<div class="items">
				<div class="itemname">${dto.itemName }<span class="btn_share"><button id="btnShare">공유하기</button></span></div>
				...
				<div class="iteminfo">
					<dl class="list2">
						...
						<dd class="desc">${dto.unit }</dd>
					</dl>
					<dl class="list2">
						<dt class="tit">중량/용량</dt>
						<dd class="desc">${dto.kg }</dd>
					</dl>
					<dl class="list2">
					...
					<dl class="list2">
						...
						<dd class="desc">
							냉동/스티로폼 <strong> 택배배송은 에코포장이 스티로폼으로 대체됩니다.</strong>
						</dd>
					</dl>
					<c:if test="${!empty dto.allegy }">
					<dl class="list2">
						<dt class="tit">알레르기정보</dt>
						<dd class="desc">${dto.allegy }</dd>
					</dl>
					</c:if>
					<dl class="list2">
						<dt class="tit">안내사항</dt>
						<dd class="desc">${dto.comments }</dd>
					</dl>
					...
					<dl class="list2">
						...
						<input type="button" class="cart_img" value=" 장바구니담기 " onclick="toBasket();" />
						<%-- <c:choose>
							<c:when test="${empty sessionScope.basket.itemNum }">
								<b>로그인하면 회원가격.</b>
								<br />
								<br />
							</c:when>
							<c:otherwise>
								${sessionScope.basket.itemNum }님 ${sessionScope.basket.amount } 반갑습니다 <br />
								<br/>
							</c:otherwise>
						</c:choose> --%>
						</dd>
	...
	<div id="mainview">
		<div id="tap">
			<ul >
				<li><a href="#itemdesciption">상품설명</a>
				<li><a href="#iteminfomation">상세정보</a>
				<li><a href="#">후기</a>
				<li><a href="#">문의</a>
			</ul>
		</div>

		<div id="itemdesciption">
			<img alt="" src="${imagePath }${dto.itemNum }/${dto.image1}">
		</div>

		<div id="iteminfomation">
			<img alt="" src="${imagePath }${dto.itemNum }/${dto.image2}">
		</div>
	</div>
