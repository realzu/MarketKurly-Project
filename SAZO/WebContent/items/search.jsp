...
<link rel="stylesheet" type="text/css" href="/SAZO/css/real_main.css">

<script type="text/javascript">

	function sendIt(){		
		var f = document.search;		
		f.action = "<%=cp%>/market/search.do?num=<%=num%>";
		f.submit();		
	}
</script>
</head>
	...
	<div id="itemList"><%--페이지전체 --%>		
		...
		<div id="searchBox">
			<form action="" method="post" name="search" >
				<!-- <hr color="#5f0080" size="2"/> -->
				<font class="label">상품명</font>
				<input type="text" name="searchValue" value="${searchValue}" class="box"/>
				<input type="button" value=" 검색하기 " class="btn" onclick="sendIt();"/>	
			</form>		
		</div>
		...
		<div id="itemList_info" >
			...
			<div class="rightInfo">						
					<ul>
					<li><a href="<%=cp %>/market/search.do?num=1${params}" >인기순</a> ｜ </li>
					<li><a href="<%=cp %>/market/search.do?num=2${params}" >신상품순</a> ｜ </li>
					<li><a href="<%=cp %>/market/search.do?num=3${params}" >낮은 가격순</a> ｜ </li>
					<li><a href="<%=cp %>/market/search.do?num=4${params}" >높은 가격순</a></li>
					</ul>
			</div>
		</div>
